...
export class WorkerService implements OnModuleInit {
  ...
  private async executeScrapingProcess(request: RequestMetadata, scraper) {
    ...
    if (request.browserId) {
      ...
      // We'll need to create a method in BrowserPoolService to get browser by ID
  ...
  private buildQuery(parameters): AdLibraryQuery {
  ...
  stopAllWorkers() {
    ...
    this.logger.log(`Stopping all workers (${this.activeWorkers} active)`);
  }
}
