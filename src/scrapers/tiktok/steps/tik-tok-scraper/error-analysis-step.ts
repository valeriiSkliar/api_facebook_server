    ...
    try {
      ...
      if (analysisResults.length > 0) {
        ...
        if (analysisResults.length > 0) {
          ...
          this.logger.debug(`Sample analysis result to be saved:`, {
            ...
            errorMessage: sample.errorMessage
              ? sample.errorMessage.substring(0, 100)
              : 'None',
        ...
        try {
          ...
          this.logger.debug(
            `Details of save error:`,
            saveError instanceof Error
              ? { message: saveError.message, stack: saveError.stack }
              : saveError,
          );
        ...
        if (context.state.apiErrors && context.state.apiErrors.length > 0) {
          this.logger.debug(
            `${context.state.apiErrors.length} API errors exist in context but no valid analysis results were generated`,
          );
        }
      }
